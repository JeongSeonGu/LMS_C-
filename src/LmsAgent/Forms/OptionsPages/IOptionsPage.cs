...
public interface IOptionsPage
{
    ...
    /// <see cref="ValidateSettings"/>에서 먼저 걸러집니다.</summary>
    void SaveTo(AppSettings settings);

    /// <summary>저장 전 검증. 문제가 있으면 사용자에게 보여줄 메시지를 반환하고, 문제 없으면 null을 반환합니다.
    /// (이름을 Validate로 하면 UserControl이 상속하는 ContainerControl.Validate()와 충돌하므로 다른 이름을 씁니다.)</summary>
    string? ValidateSettings() => null;
}
