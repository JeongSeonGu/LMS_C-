...
- 교무업무 페이지 (환경설정 &gt; 네트워크의 주소를 기본 브라우저로 엶)
- **학사 일정**
  - 일정 등록...
...
| 대분류 | 항목 |
|---|---|
| 일반 | 학교명(로그인 시 서버 값으로 최초 자동 채움), Windows 시작 시 자동 실행, **첫 시작 시 공지사항 알림**(기본 미체크), **쉬는 시간 전자칠판 페이지 활성화**·출력 모니터·**쉬는 시간 종료안내** |
...
환경설정 &gt; 라이센스에서 학교에 발급된 인증키를 입력합니다. `LicenseGuardService`가
...
업무 일지 창의 헤더, 닫기(×) 버튼 바로 왼쪽에 **🗒 버튼**이 있습니다. 누르면 목록 화면이
사라지고 그 자리에 빈 메모장이 나타나 — 실제 Windows **스티커 메모**처럼 자유롭게 쓸 내용을
적을 수 있습니다. 다시 누르면 목록으로 돌아갑니다.

- 서버와 주고받지 않는 **순수 로컬 메모**입니다(`Services/WorkNoteStore.cs`,
  `%AppData%\LmsAgent\worknote.txt`). 다른 PC나 웹 화면과 동기화되지 않습니다.
- 입력을 멈추면 0.8초 뒤 자동 저장되고, 메모 화면을 닫을 때도 즉시 한 번 더 저장합니다.
...
프로그램은 실행될 때마다 환경설정 &gt; 네트워크의 "업데이트 서버" 주소에서 버전 매니페스트
JSON을 조회합니다(`Services/UpdateService.cs`). 매니페스트 형식은 다음과 같습니다.
...
```json
{
  ...
  "notes": "변경 사항 설명(선택, 현재 UI에는 표시하지 않음)",
  "mandatory": false
}
```
...
4. 그 다음부터 프로그램을 실행하는 모든 PC가 실행 시점에 새 버전을 감지해서 자동으로
   ...
   (5) 새 exe를 다시 실행 → (6) 배치 스크립트 자기 자신을 삭제.
   사용자는 트레이 메뉴의 "업데이트 확인..."으로 이 과정을 수동으로도 실행할 수 있습니다.
- `sha256`을 비워두면 해시 검증 없이 그대로 적용합니다(내부망 등 신뢰된 배포 경로에서만
  권장). `mandatory`는 현재 클라이언트 로직에서 별도로 사용하지 않는 예약 필드입니다.
