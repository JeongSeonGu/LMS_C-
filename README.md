...
| 대분류 | 항목 |
|---|---|
| 일반 | Windows 시작 시 자동 실행, **첫 시작 시 공지사항 알림**(기본 체크), **쉬는 시간 전자칠판 페이지 활성화**·출력 모니터·**쉬는 시간 종료안내** |
...
항목의 상세 주소로 바로 들어가야 하므로, `loginUrl`에 `next=<상세 주소의 상대경로>`
쿼리 파라미터를 덧붙여서 엽니다 — 서버 팀이 `sso_login.php`에 이 기능을 추가로 반영해
주셨습니다(SSO 자동 로그인 적용 안내.md §5-7). **`next`는 반드시 이 서비스 안의
상대경로(`/SchoolWork/WorkSupport/...`)여야 합니다** — 오픈 리다이렉트 방지를 위해
서버가 스킴이 있는 절대 URL은 전부 무시하고 대시보드로 보내기 때문에, C# 쪽에서도 절대
URL이 아니라 상대경로를 그대로 보냅니다. 토큰이 만료·중복사용 등으로 실패해 로그인
화면으로 돌아가는 경우에도, 서버가 `redirect=<next>`를 함께 붙여줘서 사용자가 그 화면에서
수동으로 로그인하면 원래 보려던 페이지로 이어서 이동합니다.

### 세션 감시(로그아웃 동기화)

C#은 두 가지 방법으로 "다른 곳에서 로그아웃됨"을 반영합니다.

1. **실시간(즉시)** — 서버가 `ws_logout.php` 호출 시(웹이든 C#이든 어느 쪽에서든) 그 계정의
   개인 Room(`user:<id>`)으로 `work.session.revoked` 도메인 이벤트를 발행해 줍니다
   (서버 팀이 추가로 반영). `TrayApplicationContext.OnDomainEventReceived`가 이 타입을
   받으면 즉시 `SessionManager.Clear()`를 호출하고 "다른 곳에서 로그아웃되어 연결이
   해제되었습니다" 풍선 알림을 띄웁니다. 별도 구독 설정 없이도 로그인 시 자동으로 들어가
   있는 개인 Room을 통해 옵니다. 이 이벤트는 **강제 로그아웃이 아니라 알림**입니다 — 서버가
   다른 세션(C#이나 다른 브라우저 탭)을 직접 끊지는 않으며, 그 세션은 원래대로 자연 만료될
   때까지 서버 쪽에서는 계속 유효합니다. C#은 이 알림을 받고 스스로 화면만 로그아웃 상태로
   맞춥니다.
2. **폴백(최대 3분 지연)** — `SessionWatcherService`가 로그인해 있는 동안 3분마다
   `ws_me.php`로 C#의 세션이 여전히 유효한지 확인합니다. 위 실시간 이벤트를 놓치거나
   웹소켓 연결이 끊긴 상태여도, 세션 자체가 만료됐다면 이 폴백으로 최대 3분 안에는
   반영됩니다.

두 경우 모두 로그인 화면·업무 일지("🔒 아직 로그인 전입니다")까지 자동으로 로그아웃
상태로 되돌아갑니다. 자세한 이벤트 형식과 처리 절차는 `docs/연동방법_및_규칙.md`와
`docs/웹소켓_데이터통신규칙.md` §7-B를 참고하세요.

### 공지 알림 토스트

교무업무 웹페이지에서 사용자가 교직원에게 공지사항·요청사항을 등록하며 "알림"을 체크하면,
그 대상 중 로컬 프로그램으로 로그인되어 있는 사람에게 웹소켓으로 곧바로 Windows 풍선
토스트가 뜹니다(재조회를 기다릴 필요가 없습니다). 서버가 `work.notice.created` 이벤트에
표시용 `message: { title, body, link }` 필드를 함께 실어 보내 줍니다
(`docs/웹소켓_데이터통신규칙.md` §7-C).

- `TrayApplicationContext.OnDomainEventReceived`가 `message.title`이 있는 경우에만 토스트를
  띄우고, 없으면(구버전 서버이거나 알림을 체크하지 않고 등록한 경우) 조용히 무시합니다.
- 이 토스트는 **기존의 "알림" 배지·업무 일지 새로고침 로직을 대체하지 않습니다** — 그
  로직(`OnRealtimeScopeChanged`의 `notice` scope 처리)은 그대로 계속 동작하고, 이 토스트는
  그 위에 얹히는 "즉시 알려주는" 부가 기능입니다.
- 토스트를 클릭하면 `message.link`를 위 SSO(자동 로그인) 방식 그대로 열어서, 로그인 상태라면
  다시 로그인 화면 없이 바로 그 글로 이동합니다.
- Windows의 `NotifyIcon.BalloonTipClicked`는 이 토스트·일정 알림·담당업무 변경 알림이 모두
  공유하는 하나의 이벤트라서, 다른 풍선을 띄우는 곳에서는 먼저 "다음 클릭에 열 링크"를
  비워 둡니다 — 방금 뜬 무관한 풍선을 눌렀는데 예전 공지 링크가 열리는 일이 없도록 합니다.
...
헤더 제목에는 환경설정 &gt; 업무의 출력 단위에 맞춰 **조회 범위의 날짜**를 함께 보여줍니다
(`WorkJournalForm.SetHeaderRange`) — 일 단위면 `업무 일지(2026.09.25)`, 주 단위면
`업무 일지(2026.09.21 ~ 2026.09.27)` 형식입니다. 지금 보고 있는 목록이 정확히 어느
기간 것인지 한눈에 알 수 있습니다.
...
> 행간이 고르지 않게 보였습니다(학사일정·할일 행은 `AutoSize`를 끄고 `Height`만 직접
> 관리하도록 정리). 요청사항/알림/법정연수 행은 처음엔 제목과 상태 배지를
> `"[기한초과] 제목"`처럼 한 문자열로 이어 붙였는데, 그러면 분류·상태와 실제 내용이
> 하나로 뭉쳐 보여 구분하기 어려웠습니다 — 지금은 중첩 `FlowLayoutPanel`(제목 한 줄 +
> 배지가 있으면 그 아래 줄에 색이 있는 배지)로 나눠서, 레이아웃 엔진이 각 줄 높이를
> 전담하고 제목과 상태가 시각적으로도 분리되도록 고쳤습니다.
...
환경설정 &gt; 일반의 "첫 시작 시 공지사항 알림"을 켜면(기본값 **체크**), **로그인에 성공할
때마다**(시간대 제한 없음) 아래 6가지를 모아 보여주는 요약 팝업(`StartupSummaryForm`)이
뜹니다. 팝업 하단의 "오늘은 보지 않기"를 체크하면 그날(`StartupNoticeSuppressedDate`)만
건너뛰고, 다음 날 로그인하면 다시 뜹니다.
...
(`Services/UpdateService.cs`). 기본값은 `https://future-class.kr/LmsAgent/update/manifest.json`
입니다(`AppSettings.UpdateManifestUrl`) — 다른 주소를 쓰려면 환경설정에서 직접 바꾸면 됩니다.
매니페스트 형식은 다음과 같습니다.
