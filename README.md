...
- 헤더 오른쪽에는 로그인 중일 때만 **"ID: 아이디  이름: 이름"** 표시와 **로그아웃** 버튼이
  나타납니다. 로그아웃을 누르면 서버에도 알려서(`ws_logout.php`, 다른 화면에도
  `work.session.revoked`로 전파됨) 이 창은 닫히고 로컬 세션도 즉시 정리됩니다.
...
- 대분류 하위 항목들은 세로로 긴 목록이 아니라, 모서리가 둥근 **정사각형/직사각형 타일
  버튼**이 가로로 나열되다가 폭이 차면 다음 줄로 넘어가는 카드 그리드로 보여줍니다
  (`UiTheme.StyleLiquidTileButton`) — 마우스를 올리면 배경이 진한 하늘색으로, 글자가
  흰색으로 바뀝니다.
...
메뉴 순서(우클릭 메뉴와 왼쪽 클릭 창의 사이드바 모두 동일한 순서를 씁니다):
...
`업무 일지(09.21 ~ 09.27)` 형식입니다. 주 단위는 창 폭이 좁아 연도까지 넣으면 문구
전체가 잘려 보여서 연도를 뺐습니다. 지금 보고 있는 목록이 정확히 어느 기간 것인지
한눈에 알 수 있습니다.
...
속하지 않는 세 가지 업무도 함께 정리해 보여줍니다 — **분류별로 묶어서** 보여주며, 링크가
있는 항목은 앞에 화살표(▶)가 붙습니다. **클릭한다고 바로 웹 페이지로 넘어가지는 않습니다**
— 먼저 ▼로 바뀌며 그 아래 "[세부 페이지 이동]" 링크가 나타나고, 그 링크를 다시 클릭해야
실제로 브라우저가 열립니다(제목을 클릭했는데 곧바로 페이지가 튀어나가 당황스럽다는
피드백을 반영).
...
- **서버(DB)·웹 화면과 전혀 통신하지 않는, 이 PC에만 있는 기록입니다**
  ...
  자동으로 합쳐지거나 동기화되지 않습니다 — 등록 창(목록/수정 창 모두) 하단에 항상
  **빨간 글씨**로 "개인일정은 로컬에만 기록될 뿐 학사 일정과 연동이 되지 않습니다"라고
  안내합니다.
...
> ⚠ **과거 구현 버그(수정됨)**: 등록 목록 창의 안내 문구를 `Width=350, Height=20`
> 고정 크기 `Label`에 그대로 넣었는데, 실제 문구 폭이 그보다 넓어 자동으로 두 줄로
> 줄바꿈되면서 `Height`가 한 줄 분량뿐이라 둘째 줄("않습니다.")이 그대로 잘려 보였습니다.
> 지금은 `Dock=Bottom`으로 패널 전체 너비를 쓰게 해서 한 줄에 다 들어가도록 고쳤고,
> 폭이 좁은 등록/수정 창은 두 줄 분량의 `Height`를 넉넉히 줘서 줄바꿈되어도 잘리지
> 않습니다.
...
- **로그인하면 예전에 써 둔 메모가 자동으로 함께 나타납니다.** 🗒 버튼을 다시 눌러야만
  예전 메모가 보이던 문제(수정됨) — 저장된 메모가 있으면 로그인 성공 시
  `StickyNoteService.RestoreIfAny()`가 자동으로 복원해서 보여줍니다. `ShowAll()`(🗒 버튼
  전용)과 달리, 저장된 메모가 하나도 없을 때는 빈 메모를 새로 만들지 않습니다 — 한 번도
  메모를 써 본 적 없는 사용자에게 로그인할 때마다 빈 메모 창이 갑자기 나타나면 안 되기
  때문입니다.
